 ...
 */
public class MyMapOrListWriter {

  final BaseWriter.MapWriter map;
  final BaseWriter.ListWriter list;

  MyMapOrListWriter(final BaseWriter.MapWriter writer) {
    this.map = writer;
    this.list = null;
  }

  MyMapOrListWriter(final BaseWriter.ListWriter writer) {
    this.map = null;
    this.list = writer;
  }
  ...
  MyMapOrListWriter map(final String name) {
    assert map != null;
    return new MyMapOrListWriter(map.map(name));
  }

  MyMapOrListWriter listoftmap(final String name) {
    assert list != null;
    return new MyMapOrListWriter(list.map());
  }

  MyMapOrListWriter list(final String name) {
    assert map != null;
    return new MyMapOrListWriter(map.list(name));
  }
